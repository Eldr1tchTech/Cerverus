...
void pool_allocator_free(pool_allocator *pool_alloc, void *block) {
  if (block < pool_alloc->data ||
      block > pool_alloc->data +
                  pool_alloc->stride * (pool_alloc->availability->size - 1)) {
